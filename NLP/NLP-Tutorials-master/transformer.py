...
def export_attention(name="transformer"):
    with open("./visual/tmp/transformer_v2i_i2v.pkl", "rb") as f:
        dic = pickle.load(f)
    model = Transformer(MODEL_DIM, MAX_LEN, N_LAYER, N_HEAD, 27, DROP_RATE)
    model.load_weights("./visual/models/transformer/model.ckpt")
    return model, dic["v2i"], dic["i2v"]
...
if __name__ == "__main__":
    ...
    #
    train(m, d, step=1000)
    m, v2i, i2v = export_attention()
    result = m.translate('23-07-98', d.v2i, d.i2v)
    print(result)